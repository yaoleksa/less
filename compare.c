...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < bi.biWidth; j++)
        {
            ...
            // read RGB triple to outfile
            fread(&triple2, sizeof(RGBTRIPLE), 1, outptr);
            if(triple1.rgbtRed != triple2.rgbtRed || triple1.rgbtGreen != triple2.rgbtGreen || 
            triple1.rgbtBlue != triple2.rgbtBlue)
            {
            	fprintf(stderr, "Differ at position: ");
            	printf("i = %d j = %d\n", i, j);
            	return 7;
            }
        }
